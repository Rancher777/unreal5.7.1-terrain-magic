...
{
	// Find the landscape chunk containing this clip's location
	...
	ALandscape* ClipLandscape = nullptr;
	...
	FVector ClipLocation = GetActorLocation();
	...
	if (!IsValid(ClipLandscape) && AllLandscapes.Num() > 0)
	{
		ClipLandscape = Cast<ALandscape>(AllLandscapes[0]);
	}
	
	FVector ClipLandscapeLocation = {};
	FVector ClipLandscapeScale = {100.0, 100.0, 100.0};
	FVector ClipLandscapeSize = {};
	
	if (IsValid(ClipLandscape))
	{
		// Get landscape transform and size
		ClipLandscapeLocation = ClipLandscape->GetActorLocation();
		ClipLandscapeScale = ClipLandscape->GetActorScale3D();
		
		// Calculate landscape size from components
		...
		if (Components.Num() > 0)
		{
			...
			ClipLandscapeSize = FVector(TotalSizeX, TotalSizeY, 0);
		}
		else
		{
			// Fallback: use default size if no components
			ClipLandscapeSize = FVector(1000, 1000, 0);
		}
	}
	else
	{
		// Fallback: if no landscape found, try to use existing values or defaults
		if (LandscapeSize.X > 0 && LandscapeSize.Y > 0 && LandscapeScale.X > 0 && LandscapeScale.Y > 0)
		{
			// Use existing values if they're valid
			ClipLandscapeLocation = LandscapeLocation;
			ClipLandscapeScale = LandscapeScale;
			ClipLandscapeSize = LandscapeSize;
		}
		else
		{
			// Last resort: use defaults
			ClipLandscapeLocation = FVector(0, 0, 0);
			ClipLandscapeScale = FVector(100.0, 100.0, 100.0);
			ClipLandscapeSize = FVector(1000, 1000, 0);
		}
	}
	
	// Update the clip's landscape information
	LandscapeLocation = ClipLandscapeLocation;
	LandscapeScale = ClipLandscapeScale;
	LandscapeSize = ClipLandscapeSize;
	
	// Calculate center location and set transform
	const FVector CenterLocation = ClipLandscapeLocation + ((ClipLandscapeSize / 2) * ClipLandscapeScale);
	InitialZPosition = CenterLocation.Z;

	const FVector ScaleFactorFromLandscapeScale = ClipLandscapeScale / 100.0;
	...
	SetClipBaseSize(FVector2D(ClipLandscapeSize.X, ClipLandscapeSize.Y));
	_Invalidate();
}
