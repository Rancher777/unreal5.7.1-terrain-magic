...
{
	// World Partition: Find the landscape chunk containing this location
	ALandscape* TargetLandscape = FindLandscapeAtLocation(Location);
	if (!IsValid(TargetLandscape))
	{
		return { false };
	}
	
	// Get transform for the target landscape
	FTransform TargetTransform = TargetLandscape->GetActorTransform();
	
	// For World Partition, we need to recalculate based on the specific landscape chunk
	// This is a simplified version - in a full implementation, you'd cache per-chunk data
	const FVector RelativeLocation = Location - TargetTransform.GetLocation();
	const FVector RelativeLocationAfterScaleDown = RelativeLocation / TargetTransform.GetScale3D();
	...
	// Note: For World Partition, PaintLayerData would need to be per-chunk
	// This is a compatibility layer that uses the first landscape's data
...
void ATerrainMagicManager::TogglePreview()
{
	...
	// World Partition: Handle multiple landscape chunks
	// For preview, we'll hide the first/main landscape chunk
	// In World Partition setups, you may want to handle this differently
	TArray<ALandscape*> Landscapes = GetAllLandscapeChunks();
	
	if (Landscapes.Num() > 0)
	{
		ALandscape* Landscape = Landscapes[0]; // Use first landscape for preview mode
		
		if (bShowPreviewMesh && IsValid(Landscape))
		{
                                      ...
                                      const FIntPoint InputRenderTargetSize)
{
	...
	// Refresh landscape chunks cache
	LandscapeChunks = GetAllLandscapeChunks();
}

void ATerrainMagicManager::InitializeForLandscape(ALandscape* Landscape)
{
	...
	// Get landscape bounds and transform
	FVector LandscapeLocation = Landscape->GetActorLocation();
	FVector LandscapeScale = Landscape->GetActorScale3D();
	
	// Get landscape component count to determine size
	const TArray<ULandscapeComponent*>& LandscapeComponents = Landscape->LandscapeComponents;
	if (LandscapeComponents.Num() > 0)
	{
		// Landscape component size is typically 63x63 quads, or component size * subsections
		ULandscapeComponent* FirstComponent = LandscapeComponents[0];
		if (IsValid(FirstComponent))
		{
			...
			int32 NumSubsections = Landscape->NumSubsections;
			
			// Calculate total landscape size in quads
			...
			for (ULandscapeComponent* Component : LandscapeComponents)
			{
				if (IsValid(Component))
				{
					...
					MinKey.X = FMath::Min(MinKey.X, SectionBase.X);
					MinKey.Y = FMath::Min(MinKey.Y, SectionBase.Y);
					MaxKey.X = FMath::Max(MaxKey.X, SectionBase.X);
					MaxKey.Y = FMath::Max(MaxKey.Y, SectionBase.Y);
				}
			}
			
			NumComponentsX = (MaxKey.X - MinKey.X) / ComponentSizeQuads + 1;
			NumComponentsY = (MaxKey.Y - MinKey.Y) / ComponentSizeQuads + 1;
			
			int32 TotalSizeX = NumComponentsX * ComponentSizeQuads;
			int32 TotalSizeY = NumComponentsY * ComponentSizeQuads;
			
			FTransform NewTransform(Landscape->GetActorRotation(), LandscapeLocation, LandscapeScale);
			this->Initialize(NewTransform, FIntPoint(TotalSizeX, TotalSizeY), FIntPoint(TotalSizeX + 1, TotalSizeY + 1));
		}
	}
	else
	{
		// Fallback: use actor transform with default size
		FTransform NewTransform(Landscape->GetActorRotation(), LandscapeLocation, LandscapeScale);
		this->Initialize(NewTransform, FIntPoint(1000, 1000), FIntPoint(1001, 1001));
	}
	
	// Update chunks cache
	if (!LandscapeChunks.Contains(Landscape))
	{
		LandscapeChunks.Add(Landscape);
	}
}

TArray<ALandscape*> ATerrainMagicManager::GetAllLandscapeChunks() const
{
	TArray<AActor*> AllActors;
	TArray<ALandscape*> AllLandscapes;
	
	if (GetWorld() != nullptr)
	{
		UGameplayStatics::GetAllActorsOfClass(GetWorld(), ALandscape::StaticClass(), AllActors);
...
ALandscape* ATerrainMagicManager::FindLandscapeAtLocation(const FVector& WorldLocation)
{
	TArray<AActor*> AllActors;
	TArray<ALandscape*> AllLandscapes;
	...
	// Find landscape containing the location
	for (ALandscape* Landscape : AllLandscapes)
	{
		if (IsValid(Landscape))
		{
			FBox LandscapeBounds = Landscape->GetComponentsBoundingBox(true);
			if (LandscapeBounds.IsInsideXY(WorldLocation))
			{
				return Landscape;
			}
		}
	...
	// Fallback: return first landscape if any exist
	return AllLandscapes.Num() > 0 ? AllLandscapes[0] : nullptr;
}
...
UTextureRenderTarget2D* ATerrainMagicManager::RenderLandscapeClipsHeightMap(UTextureRenderTarget2D* InputHeightMap)
{
	...
	{
		// World Partition: Find the landscape chunk this clip belongs to
		ALandscape* ClipLandscape = FindLandscapeAtLocation(LandscapeClip->GetActorLocation());
		FTransform ClipLandscapeTransform = LandscapeTransform;
		FIntPoint ClipLandscapeSize = LandscapeSize;
		
		if (IsValid(ClipLandscape))
		{
			ClipLandscapeTransform = ClipLandscape->GetActorTransform();
			// Get landscape size from the chunk
			const TArray<ULandscapeComponent*>& Components = ClipLandscape->LandscapeComponents;
			if (Components.Num() > 0)
			{
				int32 ComponentSizeQuads = ClipLandscape->ComponentSizeQuads;
				FIntPoint MinKey(MAX_int32, MAX_int32);
				FIntPoint MaxKey(MIN_int32, MIN_int32);
				...
				int32 NumComponentsX = (MaxKey.X - MinKey.X) / ComponentSizeQuads + 1;
				int32 NumComponentsY = (MaxKey.Y - MinKey.Y) / ComponentSizeQuads + 1;
				ClipLandscapeSize = FIntPoint(NumComponentsX * ComponentSizeQuads, NumComponentsY * ComponentSizeQuads);
			}
		}
		
		// Set landscape information where actor can use to position it relative to the landscape
		LandscapeClip->LandscapeLocation = bShowPreviewMesh? LandscapeLocationBeforePreview : ClipLandscapeTransform.GetLocation();
		LandscapeClip->LandscapeScale = ClipLandscapeTransform.GetScale3D();
		LandscapeClip->LandscapeSize = FVector(ClipLandscapeSize.X, ClipLandscapeSize.Y, 0);
		...
		// Use the clip's landscape transform for this clip
		Params.Push({"HeightRT", BufferRenderTargetForHeight});
		Params.Push({"LandscapeLocation", LandscapeClip->LandscapeLocation});
		Params.Push({"LandscapeScale", LandscapeClip->LandscapeScale});
		Params.Push({"LandscapeSize", LandscapeClip->LandscapeSize});
	...
	UTextureRenderTarget2D* InputWeightMap)
{
	...
	for (ALandscapeClip* LandscapeClip: LandscapeClips)
	{
		...
		// Set landscape information where actor can use to position it relative to the landscape
		LandscapeClip->LandscapeLocation = ClipLandscapeTransform.GetLocation();
		LandscapeClip->LandscapeScale = ClipLandscapeTransform.GetScale3D();
		LandscapeClip->LandscapeSize = FVector(ClipLandscapeSize.X, ClipLandscapeSize.Y, 0);
